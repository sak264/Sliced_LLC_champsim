...
do
    ...
    then
        ...
        # echo $(grep "cpu0_LLC TOTAL" ${reversed_file_name}) | cut -d ' ' -f 4,6,8 >> $preprocessed_file_name
        # echo $(grep "cpu1_LLC TOTAL" ${reversed_file_name}) | cut -d ' ' -f 4,6,8 >> $preprocessed_file_name
        # echo $(grep "cpu${i}_LLC AVERAGE" ${reversed_file_name}) | cut -d ' ' -f 5 >> $preprocessed_file_name
        line_count=0
        not_req_line_count=$((i))
        grep "cpu0_LLC TOTAL" ${reversed_file_name} | while read -r line
        do
            # echo $line_count $not_req_line_count
            if [[ $line_count -eq ${not_req_line_count} ]]
            then
            	((line_count++))
            	continue
            else
                # echo $line
                echo $line | grep "cpu0_LLC TOTAL" |  cut -d ' ' -f 4,6,8 >> $preprocessed_file_name
                break
            fi
            # ((line_count++))
        done
        line_count=0
        not_req_line_count=$((i))
        grep "cpu1_LLC TOTAL" ${reversed_file_name} | while read -r line
        do
            # echo $line_count $not_req_line_count
            if [[ $line_count -eq ${not_req_line_count} ]]
            then
                ...
                # echo $line
                echo $line | grep "cpu1_LLC TOTAL" |  cut -d ' ' -f 4,6,8 >> $preprocessed_file_name
                break
            fi
            # ((line_count++))
        done
...
python3 ./process_data.py $preprocessed_file_name $cores
preprocessed_folder="preprocess_result"
[ -d ${preprocessed_folder} ] || mkdir ${preprocessed_folder}
mv $preprocessed_file_name ${preprocessed_folder} 
